...
import org.joni.test.meta.FriendRequest;
...
public class MetaService extends HessianServlet implements MetaDataAPI {
	...
	/**
	 * used to pass along the user certificate from servlet handling to the
	 * actual methods doing the work
	 */
	...
	public void service(ServletRequest request, ServletResponse response) throws IOException, ServletException {
		...
		if (request instanceof HttpServletRequest) {
			...
			try {
				...
				// System.out.println("User " + new String(user.getIdentity()) +
				// " is valid");
			...
			// username = (String)
			// session.getAttribute(SPConfiguration.userIdKey);
	...
	public void updateFile(MetaFile updatedFile) throws IOException {
		...
		// TODO:if only change is directory contents or stripe or key locations,
		// should not need write permission to
		// parent dir.
		...
		if (!(updatedChildren == null && oldChildren == null) && !(updatedChildren != null && updatedChildren.equals(oldChildren))) {
			throw new IOException("Cannot change child directories/files of " + updatedFile.getName()
					+ " by updating a directory, use delete or putFile.");
		}
	...
	public MetaFile getFileByPath(String path) throws IOException {
		...
		// System.out.println("st: '" + path + "' and st.size =" +
		// st.countTokens());
		...
		if (returnme == null) {
			// throw new FileNotFoundException("Root '" + token +
			// "' not found");
			return null;
		}
	...
	public MetaFile getFile(UUID id) throws IOException {
		...
		if (parentId != null) {
			...
			// TODO: requesting random files, you gain knowledge of the parent
			// dir, should prevent...
			try {
	...
	public List<String> getFriends() throws SessionException {
		...
		UserInfo thisUserInfo = users.get(thisUser);
		return thisUserInfo.getFriends();
	}
	...
	public List<FriendRequest> getFriendRequests() throws SessionException {
		String thisUser = getUser();
		UserInfo thisUserInfo = users.get(thisUser);
		return thisUserInfo.getRequests();
	}
	...
	public void acceptFriendRequest(String name) throws SessionException {
		String thisUser = getUser();
		UserInfo thisUserInfo = users.get(thisUser);
		List<FriendRequest> requests = thisUserInfo.getRequests();
		for (int i = 0; i < requests.size(); i++) {
			FriendRequest request = requests.get(i);
			if (request.getName().equals(name)) {
				// add the friend
				List<String> thisFriends = thisUserInfo.getFriends();
				thisFriends.add(name);
				// remove request
				requests.remove(i);
				users.put(thisUserInfo.getName(), thisUserInfo);
				// handle requester
				UserInfo requestingUser = users.get(name);
				List<String> requestingFriends = requestingUser.getFriends();
				requestingFriends.add(thisUser);
				users.put(requestingUser.getName(), requestingUser);
				return;
			}
		}
		try {
			writer.write("user: " + thisUser + " trying to add a friend: " + name + " that hasn't requested friendship!\n");
		} catch (IOException e) {
			// do nothing
		}
	}
	...
	public void rejectFriendRequest(String name) throws SessionException {
		...
		try {
			writer.write("user: " + thisUser + " trying to remove nonexisting friend request from: " + name + "!\n");
		} catch (IOException e) {
			// do nothing
		}
